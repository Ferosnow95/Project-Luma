...
> The repository and source code use the internal code name **SlashCursor**.
> "Glint" is the product name; the two refer to the same project.
...
# 3. Run
dotnet run --project src/SlashCursor
```
...
```powershell
dotnet publish src/SlashCursor -c Release -r win-x64 --self-contained false
```

The published app lands in
`src/SlashCursor/bin/Release/net10.0-windows/win-x64/publish/`. Double-click
`SlashCursor.exe` to run it without the SDK.

---

## First-time setup

1. **Launch** the app (`dotnet run --project src/SlashCursor`).
...
| What | Where |
|---|---|
| Settings (provider, model, capture size) | `%APPDATA%\SlashCursor\settings.json` |
| Encrypted API keys | inside `settings.json` (DPAPI, per-user) |
| Log file | `%TEMP%\slashcursor.log` |
| Saved search screenshots | `%TEMP%\SlashCursor\` |
...
```
SlashCursor.slnx                 Solution (.slnx)
src/SlashCursor/
...
- Check `%TEMP%\slashcursor.log` for details.
